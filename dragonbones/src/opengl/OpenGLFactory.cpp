...
namespace opengl
{
    ...
    void OpenGLFactory::addDBEventListener(const std::string& type, const std::function<void(EventObject*)>& listener) {
        
        //dmLogInfo("Event: called %s", type.c_str());
    }


    void OpenGLFactory::dispatchDBEvent(const std::string& type, EventObject* value) {
        //dmLogInfo("Event: dispatch %s", type.c_str());
        if(dispatchEvents && lam_listener){
            //_listener(type, value);
            lam_listener(type, value);
        }
    } 
